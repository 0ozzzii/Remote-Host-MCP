...
choose_layout() {
  ...
  PROGRESS_STATE="$CONFIG_DIR/install-progress.env"
  OWNERSHIP_STATE="$CONFIG_DIR/resource-ownership.env"
  CERTBOT_DIR="$CONFIG_DIR/certbot"
  CERTBOT_SECRET_DIR="$SECRET_DIR/dns"
  export INSTALL_MODE CODE_BASE CONFIG_DIR STATE_DIR LOG_DIR SECRET_DIR BACKUP_DIR RUNTIME_DIR RELEASES_DIR CURRENT_LINK INSTALL_STATE PROGRESS_STATE OWNERSHIP_STATE CERTBOT_DIR CERTBOT_SECRET_DIR
}

prepare_layout_dirs() {
  install -d "$CODE_BASE" "$RELEASES_DIR" "$CONFIG_DIR" "$STATE_DIR" "$LOG_DIR" "$RUNTIME_DIR"
  install -d -m 700 "$SECRET_DIR" "$BACKUP_DIR" "$CERTBOT_DIR" "$CERTBOT_SECRET_DIR"
}
