...
cd "$ROOT"
ensure_dirs
chmod +x rmcp scripts/*.sh 2>/dev/null || true

header 'Remote Host MCP 0.1.0-alpha.1 - Step 1/2: Local MCP bootstrap'
...
if [[ -x .venv/bin/python ]]; then
  ...
  python3 -m venv .venv && ok || { fail; exit 1; }
fi

step '3/9' 'Installing/updating package'
if .venv/bin/python -m pip install -q --upgrade pip &&
   .venv/bin/pip install -q -e '.[dev]'; then
  ok
else
  fail
  exit 1
...
if [[ ! -f .env ]]; then
  ...
  skip 'existing .env kept'
fi

step '5/9' 'Generating MCP Path Key'
current_key="$(mcp_env_value PATH_KEY 2>/dev/null || true)"
if [[ "$current_key" =~ ^[A-Za-z0-9_-]{32,128}$ && "$current_key" != REPLACE_* ]]; then
  skip 'existing key kept'
else
  new_key="$(python3 - <<'PY'
import secrets
...
PY
  ...
  set_mcp_env_value PATH_KEY "$new_key"
  ok 'generated and saved (not printed)'
fi

current_host="$(mcp_env_value PUBLIC_HOST 2>/dev/null || true)"
if ! valid_hostname "$current_host"; then
  set_mcp_env_value PUBLIC_HOST 'mcp.invalid'
  current_host='mcp.invalid'
fi
...
if .venv/bin/pytest -q; then
  ...
  fail 'tests failed; server was not restarted'
  exit 1
fi
...
if bash scripts/manage.sh restart >/dev/null; then
  ...
  fail
  bash scripts/manage.sh logs 80 || true
  exit 1
fi
...
if curl -fsS --max-time 5 "http://127.0.0.1:${port}/health" >/dev/null; then
  ...
  fail
  die 'Local MCP health failed. Run: bash scripts/manage.sh logs 120'
fi

printf '[9/9] Registering global rmcp command\n'
if bash scripts/register-command.sh; then
  :
else
  warn 'Global command registration failed. Local MCP remains healthy.'
fi
...
printf 'Show key / full URL later: rmcp -> option 6\n'
hr
